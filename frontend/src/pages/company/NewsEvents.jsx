...
const NewsEvents = () => {
    ...
    useEffect(() => {
        const fetchPageData = async () => {
            try {
                const res = await fetch(`${API_BASE_URL}/api/pages/news`);
    ...
    return (
        <PageMotion>
            ...
            >
                ...
                <h1 className="text-4xl md:text-5xl font-bold mb-4">{heroData?.title || 'News & Events'}</h1>
            ...
            <section className="py-16 px-6 max-w-6xl mx-auto">
                ...
                >
                    {(gridData?.news || defaultNews).map((item, i) => (
                        <motion.div key={i} variants={scaleUp} whileHover={{ y: -5 }} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
                            <div className="flex items-center gap-3 mb-4">
                                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${item.tag || 'bg-blue-100 text-blue-800'}`}>{item.category}</span>
                                <span className="text-gray-400 text-sm">{item.date}</span>
                            </div>
                            <h3 className="text-lg font-bold text-gray-800 mb-3 leading-tight">{item.title}</h3>
                            <div className="text-gray-600 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: item.desc }} />
                            <button className="mt-4 text-blue-600 text-sm font-medium hover:underline">Read More →</button>
                        </motion.div>
                    ))}
                </motion.div>
                {(gridData?.news || defaultNews).length > 6 && (
                    <div className="text-center mt-12">
                        <button className="bg-white border border-gray-300 text-gray-700 font-semibold px-8 py-3 rounded-lg hover:bg-gray-50 transition-colors">
                            Load More Updates
                        </button>
                    </div>
                )}
            ...
            <motion.section
                ...
                initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
            >
                <h2 className="text-3xl font-bold mb-3">{subscribeData?.title || 'Stay Updated with Afwan Tech'}</h2>
                <div className="text-blue-200 mb-8" dangerouslySetInnerHTML={{ __html: subscribeData?.description || '' }} />
                <div className="flex justify-center gap-3 max-w-md mx-auto">
                    ...
                    <button className={`${subscribeData?.btnColor || 'bg-yellow-400 hover:bg-yellow-300'} text-black px-6 py-3 rounded-lg font-semibold transition-colors`}>{subscribeData?.btnText || 'Subscribe'}</button>
                </div>
            </motion.section>
        </PageMotion>
    );
