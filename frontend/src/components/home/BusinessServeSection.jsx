...
const BusinessServeSection = ({ data }) => {
  const defaultBusinesses = [
    { name: "B2B", iconType: "FaBuilding" },
    { name: "Wholesale Suppliers", iconType: "FaBoxOpen" },
    { name: "Manufacturers", iconType: "FaIndustry" },
    { name: "Exporters & Importers", iconType: "FaGlobe" },
    { name: "Service Providers", iconType: "FaUserTie" },
    { name: "Healthcare", iconType: "FaHospital" },
    { name: "Education & Institutions", iconType: "FaGraduationCap" },
    { name: "Entertainment", iconType: "FaFilm" },
    { name: "Charity", iconType: "FaHandsHelping" },
  ];
  ...
  const finalRightTitle = data?.rightTitle || "Businesses We SERVE";
  const finalRightDescription1 = data?.rightDescription1 || "Building a Strong Organic Presence for Businesses & MSMEs like Yours Since 2011. Webpulse has worked with businesses from a wide range of industries, delivering Website Designing, SEO Services, Digital Marketing, and PR Services.";
  ...
  const finalBtnRightText = data?.btnRightText || "Send Enquiry";
  const finalBtnTopLink = data?.btnTopLink || "/contact";
  const finalBtnRightLink = data?.btnRightLink || "/contact";
          ...
          {finalBusinesses.map((item, index) => (
            ...
            >
              <div className="text-3xl text-[#123447] mb-4 flex justify-center">
                {getIcon(item.iconType || item.icon)}
              </div>
